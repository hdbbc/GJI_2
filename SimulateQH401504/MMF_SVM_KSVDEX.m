 ...
 close all;
 clc;
    MTOriginal=load('QH401504TS5EX.dat');
...
%% 形态滤波去噪
     MT_MMF=fcocmk_zhu(RawMT,70,0);%选择合适的结构元素大小
     High=RawMT-MT_MMF; %分离有效低频信号和含噪的高频信号
  figure(1)
     plot(t,RawMT,'b');hold on;
     plot(t,MT_MMF,'r');hold on;
...
%%高质量信号与噪声信号
   ...
   figure(4)
     ...
     fids1 = fopen('QH401504TS5EXMMFSVMGood.dat', 'wt');%保存KSVD处理后的噪声的信号
     fprintf(fids1, '%10.0f%10.0f',b);
     fclose(fids1);
     CLzong=load('QH401504TS5EXMMFSVMGood.dat');
     
     fids2 = fopen('QH401504TS5EXMMFSVMNoisy.dat', 'wt');%保存KSVD处理后的噪声的信号
     fprintf(fids2, '%10.0f%10.0f',a);
     fclose(fids2);
     CLzong=load('QH401504TS5EXMMFSVMNoisy.dat');
%% KSVD字典学习去噪
    N=length(a);
    for Step=0:00;
      Dataspan=1:N;
...
%% 保存数据
    MMFSVMKSVD=MT_MMF+Mtdata+b+average;
    MT=load('QH401504TS5EX.dat');
    MT(300001:750000)=MMFSVMKSVD;
    MMFSVMKSVD=MT;
    fids4=fopen('QH401504TS5EXMMFSVMKSVD.dat', 'wt');%保存SVM-KSVD处理后的信号
    fprintf(fids4,'%10.0f%10.0f',MMFSVMKSVD);
    fclose(fids4);
    CLzong=load('QH401504TS5EXMMFSVMKSVD.dat');
 figure
    plot(MTOriginal,'b');hold on;
    plot(MMFSVMKSVD,'r');hold on;
toc
